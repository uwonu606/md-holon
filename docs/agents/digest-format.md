...
- 걸린 자리 is generated by the check script from ids only: every claim line, every comparison row, and every other conflict page that names `<name>#C<n>` of either side. The first three columns are the script's; it fails an `open` page whose rows differ from the derived set. Places without an id are not listed.
