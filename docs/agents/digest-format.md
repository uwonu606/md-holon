...
- `stance` is optional and append-only. Each entry names a conflict page whose `status` is `decided`, and the same entry appears in both digests of that pair. `side` is one of the pair's two claims. Entries are never edited or removed; a later page that reverses an earlier one adds a new entry and leaves the old one (see Conflict page). Two entries on the same claim with opposite sides are allowed; the check script does not judge them, and the pages' `opened` dates give the order.
...
- 결과 is written by the decide session after 의견, and `status` flips to `decided` in the same edit. In that edit the session also appends the `stance` entry to both digests. Claims are never rewritten, deleted, or marked; the digest head carries the stance.
- A `decided` page must have every `action` and `reason` filled, and a matching `stance` entry in both digests; a `stance` entry whose page is missing or still `open` fails.
- Reversal: when the human sides against an earlier decided page, that earlier page is not edited. Its row in the new page's 걸린 자리 gets `kept` with a reason saying this page reverses it, and the new stance is appended beside the old one in the digest head.
- A `|` inside a quote breaks the table; pick another excerpt.

## Decide

The decide skill runs outside ingest, right after the human fills 의견 on one page. In one pass it fills `action` and `reason` on every 걸린 자리 row, writes 결과, flips `status`, appends `stance` to both digests, and runs the check script. It leaves the files uncommitted; the human reads the diff and commits.
